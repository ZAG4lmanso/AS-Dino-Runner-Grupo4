...
class  Dinosaur(Sprite):
    SOUND_JUMP = pygame.mixer.Sound('saltorex.m4a')
    X_POS = 50 
    Y_POS = 300
    JUMP_VEL = 10
    def __init__(self):
    ...
    def run(self):
        self.image = RUNNING[0] if self.dino.step <= 5 else RUNNING[1]
        print(self.dino_step)
        self.dino_rect.x = self.X_POS
        self.dino_rect.y = self._POS
        self.dino_step += 1
    ...
    def jump(self):
        ...
        if self.dino_rect.y < -self.JUMP_VEL:
            self.dino_rect.y = self.Y_POS
            self.dino_jum = False 
            self.dino_run = True
            self.jump_vel = self.JUMP_VEL
